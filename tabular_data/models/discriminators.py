...
class _netD_ess(nn.Module):
    def __init__(self, ngpu, dimx, dropout=0, leaky_inplace=False, num_classes=1):
        super(_netD_ess, self).__init__()
        self.ngpu = ngpu
        self.num_classes = num_classes
        self.main = nn.Sequential(
            nn.Linear(dimx, 1000, bias=True),
#            nn.BatchNorm1d(1000, affine=True), # added 9-8-2024
            nn.LeakyReLU(0.2, inplace=leaky_inplace),
            nn.Dropout(p=dropout),  # Add dropout layer after LeakyReLU
...
class _netD_ess_cond(nn.Module):
    def __init__(self, ngpu, n_classes, embedding_dim, dimx, dropout=0, leaky_inplace=False, num_classes=1):
        # n_classes: number of condition levels
        super(_netD_ess_cond, self).__init__()
        self.ngpu = ngpu
        self.num_classes = num_classes

        # Label-conditioned component
        self.label_condition_disc = nn.Sequential(
            nn.Embedding(n_classes, embedding_dim),
            nn.Linear(embedding_dim, 16),  # Adjust the layer size based on your need
            nn.ReLU()
        )
        self.main = nn.Sequential(
            nn.Linear(dimx + 16, 1000, bias=True),
            nn.LeakyReLU(0.2, inplace=leaky_inplace),
            nn.Dropout(p=dropout),  # Add dropout layer after LeakyReLU
            nn.Linear(1000, num_classes, bias=True)
        )
    ...
    def forward(self, input, label):
        ...
        # Concatenate label output with data input
        concat = torch.cat([input, label_output], dim=1)
